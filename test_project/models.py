...
class ChildABro(models.Model):
    sibling = models.OneToOneField(ChildA, on_delete=models.CASCADE, related_name='brother')
    brother_text = models.TextField()

class ChildB(models.Model):
    childB_text = models.TextField()
    parent = models.ForeignKey('test_project.TopLevel', on_delete=models.CASCADE, related_name='children_b')
...
class ManyToManyModelOne(models.Model):
    ...
    model_two_set = models.ManyToManyField(to='test_project.ManyToManyModelTwo', related_name='model_one_set')
...
class DeeplyNestedChild(models.Model):
    parent = models.OneToOneField(DeeplyNestedParent, on_delete=models.CASCADE, related_name='child')

class SingleChildToy(models.Model):
    owner = models.ForeignKey(DeeplyNestedChild, on_delete=models.CASCADE, related_name='toy')

class DeeplyNestedChildren(models.Model):
    parent = models.ForeignKey(DeeplyNestedParent, on_delete=models.CASCADE, related_name='children_set')

class GrandKids(models.Model):
    parent = models.ForeignKey(DeeplyNestedChildren, on_delete=models.CASCADE, related_name='children')
